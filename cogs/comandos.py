    ...
    async def imacargos(self, ctx):
        ...
        try:
            ...
            if rows_role_banned_from_user is not None:
                await ctx.send(f"Os cargos que o(a) **{user.mention}** não pode receber são:\n"
    ...
    async def removeroleban(self, ctx, *, role: discord.Role = None):
        ...
        if ctx.author.guild_permissions.administrator:
            try:
                ...
                cursor.execute(f"""
                    SELECT * FROM d_roles_adm WHERE role_id_adm = '{role.id}' and id_server = '{id_server}'
                """)
                row = cursor.fetchone()
                if row is not None:
                    cursor.execute(f"""
                        DELETE FROM d_roles_adm WHERE role_id_adm = '{role.id}'
                    """)
                    db.commit()
                    cursor.close()
                    db.close()
